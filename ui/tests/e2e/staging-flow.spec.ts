...
test.describe('E2E Staging Flow', () => {
    ...
    test('Full User Journey: Login -> PIN Setup -> Spreadsheet Selection -> Treatment Entry', async ({ page }) => {
        ...
        const pinScreen = page.locator('text=/Set a PIN|Enter PIN|Atur PIN|Masukkan PIN/i').first();
        ...
        if (await pinScreen.isVisible()) {
            const isSetup = await page.locator('text=/Set a PIN|Atur PIN/i').first().isVisible();
        ...
        // Find the input field that comes after a label containing "Patient" or "Pasien"
        const patientLabel = page.locator('label').filter({ hasText: /Patient|Pasien/i });
        const patientInput = patientLabel.locator('..').locator('input[type="text"]').first();
